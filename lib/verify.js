...
function verify(attestation, fileStreamOrShasum, keyring, cb) {
  if (typeof fileStreamOrShasum === 'string') {
    verifyWithShasum(attestation, shasum, keyring, cb);
  }
  const hash = crypto.createHash('sha256');
  hash.on('readable', () => {
    const data = hash.read();
    if (!data) return;
    verifyWithShasum(attestation, data.toString('hex'), keyring, cb);
  });
  fileStreamOrShasum.pipe(hash);
}

function verifyWithShasum(attestation, shasum, keyring, cb) {
